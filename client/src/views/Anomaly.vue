<template>
    <el-container>
        <el-header>

            <navigation>
                <router-link :to="{name: 'about'}" class="link">About Haraka</router-link>
                <router-link :to="{name: 'anomaly'}" class="link">Anomaly</router-link>
            </navigation>
        </el-header>

        <el-main>
            <div class="about">
                <h1>Anomaly Detection On Assets</h1>
            </div>

            <div v-if="error" class="error">
                {{ error }}
            </div>

            Enter the detection data points and period, then click the <strong>Anomaly Detection</strong> button.

            <br><br>
            <div id="wrapper" style="width:1020px; display:table;">
                <div id="periodDiv" style="width:100px;display:table-row;">
                    <div style="display:inline-block">
                        <label style="font-weight:bold">Period:</label>
                        <input id="periodInput" value="7" />
                        <el-button type="text" class="text-button" @click="detect()">Anomaly Detection</el-button>
                    </div>
                </div>
                <div id="inputDiv1" style="width:420px; display:table-cell;">
                    <!--<label style="font-weight:bold">Points:</label>-->
                    <textarea id="requestTextArea" class="UIInput" style="display: none;width:580px; height:800px;">
                        [
                            {
                              "Timestamp": "2018-03-01T00:00:00Z",
                              "Value": 32858923
                            },
                            {
                              "Timestamp": "2018-03-02T00:00:00Z",
                              "Value": 29615278
                            },
                            {
                              "Timestamp": "2018-03-03T00:00:00Z",
                              "Value": 22839355
                            },
                            {
                              "Timestamp": "2018-03-04T00:00:00Z",
                              "Value": 25948736
                            },
                            {
                              "Timestamp": "2018-03-05T00:00:00Z",
                              "Value": 34139159
                            },
                            {
                              "Timestamp": "2018-03-06T00:00:00Z",
                              "Value": 33843985
                            },
                            {
                              "Timestamp": "2018-03-07T00:00:00Z",
                              "Value": 33637661
                            },
                            {
                              "Timestamp": "2018-03-08T00:00:00Z",
                              "Value": 32627350
                            },
                            {
                              "Timestamp": "2018-03-09T00:00:00Z",
                              "Value": 29881076
                            },
                            {
                              "Timestamp": "2018-03-10T00:00:00Z",
                              "Value": 22681575
                            },
                            {
                              "Timestamp": "2018-03-11T00:00:00Z",
                              "Value": 24629393
                            },
                            {
                              "Timestamp": "2018-03-12T00:00:00Z",
                              "Value": 34010679
                            },
                            {
                              "Timestamp": "2018-03-13T00:00:00Z",
                              "Value": 33893888
                            },
                            {
                              "Timestamp": "2018-03-14T00:00:00Z",
                              "Value": 33760076
                            },
                            {
                              "Timestamp": "2018-03-15T00:00:00Z",
                              "Value": 33093515
                            },
                            {
                              "Timestamp": "2018-03-16T00:00:00Z",
                              "Value": 29945555
                            },
                            {
                              "Timestamp": "2018-03-17T00:00:00Z",
                              "Value": 22676212
                            },
                            {
                              "Timestamp": "2018-03-18T00:00:00Z",
                              "Value": 25262514
                            },
                            {
                              "Timestamp": "2018-03-19T00:00:00Z",
                              "Value": 33631649
                            },
                            {
                              "Timestamp": "2018-03-20T00:00:00Z",
                              "Value": 34468310
                            },
                            {
                              "Timestamp": "2018-03-21T00:00:00Z",
                              "Value": 34212281
                            },
                            {
                              "Timestamp": "2018-03-22T00:00:00Z",
                              "Value": 38144434
                            },
                            {
                              "Timestamp": "2018-03-23T00:00:00Z",
                              "Value": 34662949
                            },
                            {
                              "Timestamp": "2018-03-24T00:00:00Z",
                              "Value": 24623684
                            },
                            {
                              "Timestamp": "2018-03-25T00:00:00Z",
                              "Value": 26530491
                            },
                            {
                              "Timestamp": "2018-03-26T00:00:00Z",
                              "Value": 35445003
                            },
                            {
                              "Timestamp": "2018-03-27T00:00:00Z",
                              "Value": 34250789
                            },
                            {
                              "Timestamp": "2018-03-28T00:00:00Z",
                              "Value": 33423012
                            },
                            {
                              "Timestamp": "2018-03-29T00:00:00Z",
                              "Value": 30744783
                            },
                            {
                              "Timestamp": "2018-03-30T00:00:00Z",
                              "Value": 25825128
                            },
                            {
                              "Timestamp": "2018-03-31T00:00:00Z",
                              "Value": 21244209
                            },
                            {
                              "Timestamp": "2018-04-01T00:00:00Z",
                              "Value": 22576956
                            },
                            {
                              "Timestamp": "2018-04-02T00:00:00Z",
                              "Value": 31957221
                            },
                            {
                              "Timestamp": "2018-04-03T00:00:00Z",
                              "Value": 33841228
                            },
                            {
                              "Timestamp": "2018-04-04T00:00:00Z",
                              "Value": 33554483
                            },
                            {
                              "Timestamp": "2018-04-05T00:00:00Z",
                              "Value": 32383350
                            },
                            {
                              "Timestamp": "2018-04-06T00:00:00Z",
                              "Value": 29494850
                            },
                            {
                              "Timestamp": "2018-04-07T00:00:00Z",
                              "Value": 22815534
                            },
                            {
                              "Timestamp": "2018-04-08T00:00:00Z",
                              "Value": 25557267
                            },
                            {
                              "Timestamp": "2018-04-09T00:00:00Z",
                              "Value": 34858252
                            },
                            {
                              "Timestamp": "2018-04-10T00:00:00Z",
                              "Value": 34750597
                            },
                            {
                              "Timestamp": "2018-04-11T00:00:00Z",
                              "Value": 34717956
                            },
                            {
                              "Timestamp": "2018-04-12T00:00:00Z",
                              "Value": 34132534
                            },
                            {
                              "Timestamp": "2018-04-13T00:00:00Z",
                              "Value": 30762236
                            },
                            {
                              "Timestamp": "2018-04-14T00:00:00Z",
                              "Value": 22504059
                            },
                            {
                              "Timestamp": "2018-04-15T00:00:00Z",
                              "Value": 26149060
                            },
                            {
                              "Timestamp": "2018-04-16T00:00:00Z",
                              "Value": 35250105
                            }
                        ]
                    </textarea>
                </div>
                <div id="jsonOutput" style="width:600px; display:table-cell;">
                    <!--<label style="font-weight:bold">Response:</label>-->
                    <textarea id="responseTextArea" class="UIInput" style="width:580px; height:800px;"></textarea>
                </div>
            </div>
        </el-main>
    </el-container>
</template>

<script>
import JQuery from 'jquery'
let $ = JQuery
export default {
    methods: {
        detect() {
            const subscriptionKey = process.env.VUE_APP_ANOMALY_DETECTION_SUBSCRIPTION_KEY;
            const uriBase = process.env.VUE_APP_ANOMALY_ENDPOINT;
            var points = document.getElementById("requestTextArea").value;
            var period = document.getElementById("periodInput").value;

            function anomalyDetection (url, subscriptionKey, points, period) {
                var obj = new Object();
                obj.Points = JSON.parse(points);
                obj.Period = parseInt(period);
                var tsData = JSON.stringify(obj);
                // Perform the REST API call.
                $.ajax({
                    url: url,
                    // Request headers.
                    beforeSend: function (xhrObj) {
                        xhrObj.setRequestHeader("Content-Type", "application/json");
                        xhrObj.setRequestHeader("Ocp-Apim-Subscription-Key", subscriptionKey);
                    },
                    type: "POST",
                    // Request body.
                    data: tsData,
                })
                .done(function (data) {
                    // Show formatted JSON on webpage.
                    $("#responseTextArea").val(JSON.stringify(data, null, 2));
                })
                .fail(function (jqXHR, textStatus, errorThrown) {
                    // Display error message.
                    var errorString = (errorThrown === "") ? "Error. " : errorThrown + " (" + jqXHR.status + "): ";
                    // errorString += (jqXHR.responseText === "") ? "" : jQuery.parseJSON(jqXHR.responseText).message;
                    errorString += (jqXHR.responseText === "") ? "" : $.parseJSON(jqXHR.responseText).message;
                    $("#responseTextArea").val(errorString);
                    alert(errorString);
                })
            }

            // Perform the REST API call.
            anomalyDetection(uriBase, subscriptionKey, points, period);
        }
    }
}
</script>

